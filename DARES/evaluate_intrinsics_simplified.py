...
def evaluate_intrinsics(opt, ds_path, load_weights_folder, dataset_name="SCARED", pose_seq=1, filename_list_path=None):
    ...
    Returns:
        Dictionary of scale-invariant error metrics
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate camera intrinsics prediction with scale-invariant metrics")
    ...
    with open(result_file, 'w') as f_result:
        f_result.write('Scale-Invariant Intrinsics Evaluation Results:\n')
        ...
        if args.dataset == 'SCARED':
            ...
            for i, folder in zip(weight_indices, weight_folders):
                ...
                try:
                    error_metrics = evaluate_intrinsics(
                        AttnEncoderOpt, 
                    ...
                    # Use scale-adjusted matrix Frobenius norm as the main error metric
                    current_error = error_metrics['scale_adjusted_matrix_frobenius_norm_mean']
                    ...
                    f_result.write(f"Model {i} - Sequence 1 - Scale-Adjusted Matrix Norm: {current_error:.4f}\n")
                    ...
                    f_result.write(f"  Scale-adjusted cx error: {error_metrics['scale_adjusted_cx_abs_error_mean']:.2f} pixels ({error_metrics['scale_adjusted_cx_rel_error_mean']:.2f}%)\n")
                    f_result.write(f"  Scale-adjusted cy error: {error_metrics['scale_adjusted_cy_abs_error_mean']:.2f} pixels ({error_metrics['scale_adjusted_cy_rel_error_mean']:.2f}%)\n\n")
                
                except Exception as e:
                    print(f"Error evaluating model {i}: {str(e)}")
                    f_result.write(f"Model {i} - Evaluation failed: {str(e)}\n\n")
            
            print(f"SCARED Sequence 1: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}")
            f_result.write(f"SCARED Sequence 1: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}\n\n")
            ...
            if not DEBUG:
                ...
                for i, folder in zip(weight_indices, weight_folders):
                    ...
                    try:
                        ...
                        # Write per-model results
                        f_result.write(f"Model {i} - Sequence 2 - Scale-Adjusted Matrix Norm: {current_error:.4f}\n")
                        f_result.write(f"  Scale factor: {error_metrics['scale_factor_mean']:.4f}\n")
                        f_result.write(f"  Scale-adjusted fx error: {error_metrics['scale_adjusted_fx_abs_error_mean']:.2f} pixels ({error_metrics['scale_adjusted_fx_rel_error_mean']:.2f}%)\n")
                        f_result.write(f"  Scale-adjusted fy error: {error_metrics['scale_adjusted_fy_abs_error_mean']:.2f} pixels ({error_metrics['scale_adjusted_fy_rel_error_mean']:.2f}%)\n")
                        f_result.write(f"  Scale-adjusted cx error: {error_metrics['scale_adjusted_cx_abs_error_mean']:.2f} pixels ({error_metrics['scale_adjusted_cx_rel_error_mean']:.2f}%)\n")
                ...
                print(f"SCARED Sequence 2: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}")
                f_result.write(f"SCARED Sequence 2: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}\n\n")
            ...
            for i, folder in zip(weight_indices, weight_folders):
                ...
                for test_folder in tqdm(test_folders, desc=f"Model {i} - Test folders"):
                    ...
                    try:
                        ...
                        all_errors.append(error_metrics['scale_adjusted_matrix_frobenius_norm_mean'])
                ...
                if all_errors:
                    ...
                    f_result.write(f"Model {i} - SyntheticColon - Avg Scale-Adjusted Matrix Norm: {avg_error:.4f}\n")
                    f_result.write(f"  Scale factor: {np.mean(all_metrics['scale_factor_mean']):.4f}\n")
                    f_result.write(f"  Scale-adjusted fx error: {np.mean(all_metrics['scale_adjusted_fx_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_fx_rel_error_mean']):.2f}%)\n")
                    f_result.write(f"  Scale-adjusted fy error: {np.mean(all_metrics['scale_adjusted_fy_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_fy_rel_error_mean']):.2f}%)\n")
                    f_result.write(f"  Scale-adjusted cx error: {np.mean(all_metrics['scale_adjusted_cx_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_cx_rel_error_mean']):.2f}%)\n")
                    f_result.write(f"  Scale-adjusted cy error: {np.mean(all_metrics['scale_adjusted_cy_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_cy_rel_error_mean']):.2f}%)\n\n")
            
            print(f"SyntheticColon: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}")
            f_result.write(f"SyntheticColon: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}\n\n")
            ...
            for i, folder in zip(weight_indices, weight_folders):
                ...
                if all_errors:
                    ...
                    # Write per-model results with average metrics
                    f_result.write(f"Model {i} - C3VD - Avg Scale-Adjusted Matrix Norm: {avg_error:.4f}\n")
                    f_result.write(f"  Scale factor: {np.mean(all_metrics['scale_factor_mean']):.4f}\n")
                    f_result.write(f"  Scale-adjusted fx error: {np.mean(all_metrics['scale_adjusted_fx_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_fx_rel_error_mean']):.2f}%)\n")
                    f_result.write(f"  Scale-adjusted fy error: {np.mean(all_metrics['scale_adjusted_fy_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_fy_rel_error_mean']):.2f}%)\n")
                    f_result.write(f"  Scale-adjusted cx error: {np.mean(all_metrics['scale_adjusted_cx_abs_error_mean']):.2f} pixels ({np.mean(all_metrics['scale_adjusted_cx_rel_error_mean']):.2f}%)\n")
            ...
            print(f"C3VD: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}")
            f_result.write(f"C3VD: Best Model @ {best_i} with Scale-Adjusted Matrix Norm Error: {min_error:.4f}\n\n")
