...
from transformers import Dinov2Layer # TODO 这个import是错的
...
class Dinov2_vivit_Layer(Dinov2Layer):
    def __init__(self, original_layer, num_frames, hidden_dim):
        super().__init__(original_layer.config)
        # 复制原始层的权重
        self.load_state_dict(original_layer.state_dict(), strict=False)
    ...
    def forward(self, x):
        ...
        # 提取空间特征
        spatial_features = super().forward(x)  # (batch_size * num_frames, hidden_dim)
        spatial_features = spatial_features.view(batch_size, num_frames, -1)
        
        # 提取时间特征
        time_features = self.time_transformer(spatial_features)  # (batch_size, num_frames, hidden_dim)
        
        # 将时间特征与空间特征结合
        combined_features = time_features.view(batch_size * num_frames, -1)
        return combined_features

class vivit_Layer_manager:
    def __init__(self, model, num_frames, hidden_dim):
        self.model = model
        self.num_frames = num_frames
        self.hidden_dim = hidden_dim

    def replace_layers(self):
        for i, layer in enumerate(self.model.backbone.encoder.layer):
            self.model.backbone.encoder.layer[i] = Dinov2_vivit_Layer(layer, self.num_frames, self.hidden_dim)

class DARES_vivit(nn.Module):
    def __init__(self, frame_ids, other_frame_init_weight, 
                 r = [14,14,12,12,10,10,8,8,8,8,8,8], lora = ['q', 'v', 'mlp'],
                 pretrained_path = None,
                 debug_output=False):
        super(DARES_vivit, self).__init__()
        ...
        # 替换为 vivit 层
        self.vivit_manager = vivit_Layer_manager(model, num_frames=len(frame_ids), hidden_dim=384)
        self.vivit_manager.replace_layers()
...
if __name__ == "__main__":
    model = DARES_vivit(frame_ids=[0,-1,1],other_frame_init_weight = 1e-3, debug_output=True)
